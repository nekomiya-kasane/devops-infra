...
1. 身份认证与网络基础设施：
   ...
   | 4    | DNS (PowerDNS) | dns.coca.local       | 内网 .coca.local 域名解析服务。                | 10.40.40.10 | A 记录；所有客户端 DNS 指向此    |
   | 5    | PowerDNS Admin | dns-admin.coca.local | 可视化内网域名管理后台。                       | 10.40.40.40 | CNAME -> traefik                 |
   | 6    | DHCP           | -                    | 跨 VLAN 动态 IP 分配，配合 AD 自动注册主机名。 | 10.40.40.10 | 无需 DNS 记录                    |
   | 7    | TFTP           | -                    | PXE 网络引导文件传输。                         | 10.40.40.10 | 无需 DNS 记录                    |
   | 8    | iPXE Server    | pxe.coca.local       | 自动化装机与 VHDX 网络引导菜单。               | 10.40.40.40 | CNAME -> traefik                 |
